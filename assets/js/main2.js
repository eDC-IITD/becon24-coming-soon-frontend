$(document).ready(function(){
	...
	function footerFixed(){
		var topRow = parseInt($('#call-to-action').css('padding-top'),10)+$('.logo-row').outerHeight();
		var bottomRow = parseInt($('#call-to-action').css('padding-bottom'),10)+$('#call-to-action .footer').outerHeight()+parseInt($('#call-to-action .footer').css('margin-top'),10)+parseInt($('#call-to-action .footer').css('margin-bottom'),10);
		...
		if(middleHeight > middleAreaHeight){
			$('#call-to-action .footer').removeClass('fixed');
			$('.middle-area').removeClass('setMiddle');
			
		}else{
			$('#call-to-action .footer').addClass('fixed');
	...
	$(document).on('submit', '#news', function(e) {
		e.preventDefault();	
			$('#call-to-action .subscribe form .input-group #submit i').removeClass('fi flaticon-check');
			$('#call-to-action .subscribe form .input-group #submit i').addClass('fa fa-spinner fa-spin');
			$.ajax({
				url : 'mail.php',
				type : 'post',
				data : $(this).serialize(),
				success : function(data) {
					$('#call-to-action .subscribe form .input-group #submit i').addClass('fi flaticon-check');
					$('#call-to-action .subscribe form .input-group #submit i').removeClass('fa fa-spinner fa-spin');
